...
// ==========================================
  // 7. INTERACTIVE CURRICULUM CAROUSEL LOGIC
  // ==========================================
  ...
  function updateCarousel() {
    // Calculates horizontal alignment percentage dynamically
    const offset = currentIndex * -100;
    track.style.transform = `translate3d(${offset}%, 0px, 0px)`;
  }

  if (nextBtn && prevBtn && track) {
    // Forward Arrow Interaction Click Handler
    nextBtn.addEventListener('click', () => {
      if (currentIndex < totalSlides - 1) {
        currentIndex++;
      } else {
        currentIndex = 0; // Seamlessly loop back to start panel
      }
      updateCarousel();
    });

    // Backward Arrow Interaction Click Handler
    prevBtn.addEventListener('click', () => {
      if (currentIndex > 0) {
        currentIndex--;
      } else {
        currentIndex = totalSlides - 1; // Jump backward to last panel
      }
      updateCarousel();
    });
  }